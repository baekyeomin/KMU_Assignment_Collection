...
cv = CountVectorizer(preprocessor=clean_text, tokenizer=nltk.word_tokenize) 
...
save_sparse_as_coo_txt("train_count_coo.txt", X)          
save_sparse_as_coo_txt("train_tfidf_coo.txt", X2)         

#CountVectorizer가 전체 코퍼스(리뷰들)에서 어떤 단어를 몇 번째 열(column)에 매핑했는지에 대한 정보
with open("train_vocab.tsv", "w", encoding="utf-8") as f:
...
joblib.dump(tfidf_trans, "tfidf_transformer.pkl")

#test 데이터에 대해서도 파일 만들기
cv = joblib.load("countvectorizer.pkl")
tfidf_trans = joblib.load("tfidf_transformer.pkl")
...
# 저장
save_sparse_as_coo_txt("test_count_coo.txt", X_test)
save_sparse_as_coo_txt("test_tfidf_coo.txt", X_test_tfidf)

# vocab은 train과 동일하게 유지
words = cv.get_feature_names_out()
with open("test_vocab.tsv", "w", encoding="utf-8") as f:
    f.write("# col_index\ttoken\n")
    for idx, tok in enumerate(words):
        f.write(f"{idx}\t{tok}\n")
